 ...
 *
 * design_system/01_tokens/registry.json  →  registered source roots
 *         ↓
...
const REGISTRY_PATH = path.join(DS, '01_tokens', 'registry.json');
 ...
 * owner : ${ownerId}
 * Build : node ui_tooling/scripts/build-tokens.mjs
 */
`;
}
...
function globalCssHeader(files) {
  return `/* DO NOT EDIT — GENERATED FROM TOKEN SOURCE
 * Canonical iFlux Design System — readonly artifact.
 * Source: design_system/01_tokens/01_source/{${files.join(', ')}}
 * Build : node ui_tooling/scripts/build-tokens.mjs
 */
`;
}
...
function loadRegistry() {
  if (!fs.existsSync(REGISTRY_PATH)) {
    throw new Error(`Thiếu registry: design_system/01_tokens/registry.json`);
  }
...
function generatedLayout(src) {
  const root = path.join(REPO, src.generatedRoot);
  if (src.layer === 'global') {
    return {
      css: path.join(root, '01_css'),
      js: path.join(root, '02_js'),
      themes: path.join(root, '01_css', '01_themes'),
      relCss: `${src.generatedRoot}/01_css`,
      relThemes: `${src.generatedRoot}/01_css/01_themes`,
    };
  }
  return {
    css: path.join(root, 'css'),
    js: path.join(root, 'js'),
    themes: path.join(root, 'css', 'themes'),
    relCss: `${src.generatedRoot}/css`,
    relThemes: `${src.generatedRoot}/css/themes`,
  };
}

function emitOwner(pack, refLookup) {
  const { src, primitives, semantics, themes, breakpoints } = pack;
  const layout = generatedLayout(src);
  const outCss = layout.css;
  const outJs = layout.js;
  ...
  fs.mkdirSync(layout.themes, { recursive: true });
  ...
  for (const [themeName, list] of themes) {
    ...
    css += '}\n';
    fs.writeFileSync(path.join(layout.themes, `${themeName}.css`), css);
  }
