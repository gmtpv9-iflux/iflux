...
const genDir = path.join(DS, '01_tokens', '02_generated');
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ifx-gen-check-'));
fs.cpSync(genDir, path.join(tmp, 'before'), { recursive: true });
execFileSync('node', [path.join(REPO, 'ui_tooling', 'scripts', 'build-tokens.mjs')], { stdio: 'pipe' });
try {
...
const layoutCss = fs.readFileSync(path.join(DS, '02_foundation', 'layout.css'), 'utf8');
const lockedCols = [
...
if (missingCols.length) {
  console.error('[check-governance] FAIL — thiếu grid span đã khóa:', missingCols.join(' '));
  process.exit(1);
}
