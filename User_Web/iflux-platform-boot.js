...
(function (global) {
  ...
  function syncAccountProfileTabUrl(tabId) {
    try {
      ...
      if (!tabId || tabId === 'tab-timeline') url.searchParams.delete('tab');
      else url.searchParams.set('tab', String(tabId).replace(/^tab-/, ''));
  ...
  function resolveNavigationItems(modelId, ctx) {
    ...
    if (modelId === 'primary') {
      ...
      items = defs
        .filter(function (d) { return own || !d.ownOnly; })
        .map(function (d) {
          return freezeNavItem({
            key: d.key,
            label: d.label,
