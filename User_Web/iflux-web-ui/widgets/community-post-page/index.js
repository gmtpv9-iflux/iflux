...
var PUBLISH_KEY = 'com-post-detail';
...
var CORE_TIERS = [
  ...
  [
    ASSET + 'community-store.js?v=tickerNoDup20260810',
  ...
  [
    ...
    ASSET + 'community-post-page.js?v=scrollWave4early_20260811'
  ]
];
...
async function mountFromHostTree(root) {
  if (!root || !window.IfluxPageLayoutEngine) {
    if (window.console && console.warn) {
      console.warn('[WGT-COM-POST-PAGE] thiếu Layout Engine');
    }
    return;
  }
  var tree = await IfluxPageLayoutEngine.buildHostTree(root, PUBLISH_KEY);
  if (!tree || !tree.length) return;
  await mountPublishedWidgets(tree, { logPrefix: '[WGT-COM-POST-PAGE]' });
}
...
export async function mount(el) {
   ...
   * ESM cho community-post-page.js (legacy IIFE) dựng Sidebar phải canonical trong paintPost().
