...
(function (global) {
  ...
  var COLORS = null; /* deprecated — dùng data-series + --ifx-color-chart-* trong block-templates.css */
  ...
  var NAMES = {
    ...
    'chu-de': ['AI Việt Nam', 'Nghị quyết NN', 'Tăng vốn NH', 'Xuất khẩu thép', 'Giá dầu thế giới', 'EV xe điện', 'Căn hộ TP.HCM', 'Lãi suất giảm', 'FDI 2026', 'Vàng tăng giá']
  };
  ...
  var SUBTITLES = {
    ...
    'chu-de': 'Chủ đề · cường độ hiệu suất theo phiên (%)'
  };
  ...
  function valClass(v) {
    if (global.IfluxBlockTemplates) {
      return IfluxBlockTemplates.perfValClass(v);
    }
  ...
  function seriesAttr(idx) {
    if (global.IfluxBlockTemplates) {
      return String(IfluxBlockTemplates.chartSeriesIndex(idx));
    }
    return String((idx % 10) + 1);
  }
  ...
  function renderBlock(container, entityType, rangeKey) {
    ...
    series.forEach(function (s, idx) {
      svg.push(
        '<path class="ix-top10-market__line" data-series="' + seriesAttr(idx) +
        '" data-series-idx="' + idx + '" d="' +
        buildPath(s.data, scale.yMin, scale.yMax, plotW, plotH) + '"/>'
      );
    });
    ...
    var legendHtml = series.map(function (s, idx) {
      ...
      return '<div class="ix-top10-market__legend-item" data-legend-idx="' + idx + '" style="top:' + pct.toFixed(2) + '%">' +
        '<span class="ix-top10-market__legend-dot" data-series="' + seriesAttr(idx) + '"></span>' +
