...
(function (global) {
  ...
  function normalizePath(p) {
    if (global.IfluxNormalizePath) return global.IfluxNormalizePath(p);
  ...
  function detectPhysicalBase(path) {
    ...
    /* /…/binh-luan — Comments (trước nhánh bai-viet → community) */
    if (/\/binh-luan\/?$/i.test(path)) return '/User_Web/comments/';
    ...
    if (/^\/(?:tin-tuc|cong-dong)\/bai-viet\//i.test(path) || /^\/community\/posts\//i.test(path)) return '/User_Web/community/';
    if (/^\/(?:tin-tuc|cong-dong)\//i.test(path) || /^\/community\//i.test(path)) return '/User_Web/community/';
