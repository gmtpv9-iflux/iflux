 ...
 * Ownership: deprecated — seed/reference only until source deletion
 * Deletion: scheduled post-D5 — xem PhaseD-D4-Architecture-Verification.md § Catalog registry
 * CẤM import lại vào boot script hoặc consumer mới.
 *
 * ADM-SYS-003 — Danh mục trường hợp thông báo hệ thống + thẻ merge (legacy) */
(function (global) {
  'use strict';
  ...
  var MERGE_TAGS = [
    ...
    { key: 'Tên thực thể', label: 'Tên / mã thực thể đang theo dõi', example: 'HPG', group: 'Theo dõi' },
    { key: 'Loại thực thể', label: 'Cổ phiếu / Ngành / Hệ sinh thái / Câu chuyện', example: 'Cổ phiếu', group: 'Theo dõi' },
    { key: 'Tên người tương tác', label: 'Người thích / trả lời / chia sẻ', example: 'Lan Hương', group: 'Tương tác' },
    { key: 'Nội dung bình luận', label: 'Preview bình luận', example: 'Dòng tiền HPG đang mạnh…', group: 'Tương tác' },
  ];
  ...
  var CASES = [
    ...
    { id: 'USER_COMM_POST', code: 'NOTIF-USER-008', group: 'Cộng đồng · User', channel: 'In-app user', icon: 'ti-news', name: 'Bài viết mới từ người theo dõi', trigger: 'Người user đang theo dõi đăng bài viết cộng đồng mới (quan hệ người–người; khác gắn thẻ thực thể Watchlist)', tags: ['Tên người dùng', 'Tên tác giả', 'Tiêu đề bài viết'], defaultTitle: 'Bài viết mới', defaultMessage: '{Tên tác giả} vừa đăng: «{Tiêu đề bài viết}»', sampleVars: { 'Tên người dùng': 'Nguyễn Văn A', 'Tên tác giả': 'Quốc Bảo', 'Tiêu đề bài viết': 'VIC EV — VinFast và chu kỳ xe điện' } },
    ...
    { id: 'USER_WL_TAGGED_POST', code: 'NOTIF-USER-011', group: 'Theo dõi · User', channel: 'In-app user', icon: 'ti-bookmark', name: 'Bài mới gắn thẻ thực thể đang theo dõi', trigger: 'Có bài viết cộng đồng mới được gắn thẻ (tag) đúng thực thể user đang theo dõi — không gửi khi chỉ nhắc tên trong chữ', tags: ['Tên người dùng', 'Tên thực thể', 'Loại thực thể', 'Tên tác giả', 'Tiêu đề bài viết'], defaultTitle: 'Bài mới về {Tên thực thể}', defaultMessage: '{Tên tác giả} vừa đăng «{Tiêu đề bài viết}» gắn thẻ {Loại thực thể} {Tên thực thể}.', sampleVars: { 'Tên người dùng': 'Nguyễn Văn A', 'Tên thực thể': 'HPG', 'Loại thực thể': 'Cổ phiếu', 'Tên tác giả': 'Quốc Bảo', 'Tiêu đề bài viết': 'HPG — dòng tiền nội địa tăng tốc' } },
    { id: 'USER_FOLLOW_SHARE', code: 'NOTIF-USER-012', group: 'Theo dõi · User', channel: 'In-app user', icon: 'ti-share-3', name: 'Người đang theo dõi chia sẻ bài viết', trigger: 'Người user đang theo dõi thực hiện chia sẻ bài viết cộng đồng (Share Foundation)', tags: ['Tên người dùng', 'Tên người tương tác', 'Tiêu đề bài viết'], defaultTitle: '{Tên người tương tác} đã chia sẻ bài', defaultMessage: '{Tên người tương tác} vừa chia sẻ «{Tiêu đề bài viết}».', sampleVars: { 'Tên người dùng': 'Nguyễn Văn A', 'Tên người tương tác': 'Lan Hương', 'Tiêu đề bài viết': 'VIC EV — VinFast và chu kỳ xe điện' } },
    { id: 'USER_FOLLOW_ENTITY_COMMENT', code: 'NOTIF-USER-013', group: 'Theo dõi · User', channel: 'In-app user', icon: 'ti-message-plus', name: 'Người đang theo dõi bình luận gốc trên thực thể', trigger: 'Người đang theo dõi đăng bình luận gốc (không phải trả lời) trên trang chi tiết Entity', tags: ['Tên người dùng', 'Tên người tương tác', 'Tên thực thể', 'Loại thực thể', 'Nội dung bình luận'], defaultTitle: '{Tên người tương tác} bình luận về {Tên thực thể}', defaultMessage: '{Tên người tương tác} trên {Loại thực thể} {Tên thực thể}: «{Nội dung bình luận}»', sampleVars: { 'Tên người dùng': 'Nguyễn Văn A', 'Tên người tương tác': 'Đức Anh', 'Tên thực thể': 'HPG', 'Loại thực thể': 'Cổ phiếu', 'Nội dung bình luận': 'Dòng tiền HPG đang mạnh…' } },
    { id: 'USER_IX_COMMENT_LIKED', code: 'NOTIF-USER-014', group: 'Tương tác · User', channel: 'In-app user', icon: 'ti-heart', name: 'Ai đó thích bình luận của bạn', trigger: 'Người khác thích (like) bình luận mà bạn đã đăng trong một luồng', tags: ['Tên người dùng', 'Tên người tương tác', 'Nội dung bình luận'], defaultTitle: '{Tên người tương tác} đã thích bình luận của bạn', defaultMessage: '«{Nội dung bình luận}»', sampleVars: { 'Tên người dùng': 'Nguyễn Văn A', 'Tên người tương tác': 'Thu Hà', 'Nội dung bình luận': 'Mình cũng đang theo dõi vùng hỗ trợ này.' } },
    { id: 'USER_IX_COMMENT_REPLY', code: 'NOTIF-USER-015', group: 'Tương tác · User', channel: 'In-app user', icon: 'ti-message-reply', name: 'Ai đó trả lời bình luận của bạn', trigger: 'Người khác trả lời trực tiếp bình luận của bạn (không gồm trả lời bình luận của người khác trong cùng thread)', tags: ['Tên người dùng', 'Tên người tương tác', 'Nội dung bình luận'], defaultTitle: '{Tên người tương tác} đã trả lời bạn', defaultMessage: '{Tên người tương tác}: «{Nội dung bình luận}»', sampleVars: { 'Tên người dùng': 'Nguyễn Văn A', 'Tên người tương tác': 'Hoàng Nam', 'Nội dung bình luận': 'Đồng ý — chờ xác nhận volume.' } },
