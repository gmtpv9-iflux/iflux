...
(function (global) {
  ...
  function init() {
    ...
    /*
     * CẤM full render trên mọi iflux-news-change:
     * Related DailyFeed → setFeed → change → render → remount IX → "Đang tải" ↔ "Chưa có" (vòng lặp).
     * Chỉ render lại khi chưa có bài / đổi bài.
     */
    document.addEventListener('iflux-news-change', function onCommunityChange() {
      if (!root || !root.isConnected) {
        document.removeEventListener('iflux-news-change', onCommunityChange);
        return;
      }
      var ref = global.IfluxSeoUrl
        ? IfluxSeoUrl.parsePostRef()
