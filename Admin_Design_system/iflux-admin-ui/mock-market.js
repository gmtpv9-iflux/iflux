...
(function (global) {
  ...
  function getLiquiditySeries(exchange, metric, sessions, previewDemo) {
    ...
    var currentPts = pointsUpToCutoff(todaySeries, clock.cutoffMinutes, now);
    var averagePts = labelPointsFromSeries(avgSeries, now);
    ...
    return {
      ...
      clock: clock,
      data_as_of: liquidityDataAsOf(now, clock),
      slots_total: slots.length,
      slots_visible: currentSlotVals.filter(function (v) { return v != null; }).length
    };
  }

  function liquidityDataAsOf(now, clock) {
    var b = sessionBounds();
    if (clock.phase === 'post') {
      return slotTimestamp(minutesToTime(b.afternoonEnd), now);
    }
    if (clock.phase === 'lunch') {
      return slotTimestamp(minutesToTime(b.morningEnd), now);
    }
    if (clock.phase === 'pre') {
      return slotTimestamp(minutesToTime(b.morningStart), now);
    }
  ...
  function msUntilNextLiquidityUpdate(now) {
    ...
    if (clock.phase === 'post' || clock.phase === 'pre') return null;
    if (clock.phase === 'lunch') {
      var minsLunch = now.getHours() * 60 + now.getMinutes();
      var waitLunch = (b.afternoonStart - minsLunch) * 60000 - now.getSeconds() * 1000 - now.getMilliseconds();
      return waitLunch > 0 ? waitLunch : null;
    }
    var mins = now.getHours() * 60 + now.getMinutes();
    if (mins > b.afternoonEnd) return null;
    var slot = b.slotMinutes;
    var mod = now.getMinutes() % slot;
    var sec = now.getSeconds();
    var ms = now.getMilliseconds();
    var remain = (slot - mod) * 60000 - sec * 1000 - ms;
    if (remain <= 0) return getLiqUpdateMs();
    var nextSlotEnd = mins + (slot - mod);
    if (clock.phase === 'am' && nextSlotEnd > b.morningEnd) {
      var waitAm = (b.morningEnd - mins) * 60000 - sec * 1000 - ms;
  ...
  function getSnapshot() {
    syncMarketClockMeta();
    var snap = JSON.parse(JSON.stringify(SNAPSHOT));
    if (global.__ifluxMarketApiOverlay) {
      var o = global.__ifluxMarketApiOverlay;
      if (o.entities) {
        snap.entities = Object.assign({}, snap.entities, o.entities);
      }
      if (o.movers) snap.movers = o.movers;
      if (o.meta) snap.meta = Object.assign({}, snap.meta, o.meta);
    }
  ...
  function getGroupDetail(source, id) {
    ...
    var chart = getGroupChartData(source, id, tickers, 22);
    ...
    return {
      ...
      chart: chart,
      net_flow: getGroupNetFlow(source, id, tickers, 10)
    };
  }
  ...
  function groupPerformance(tickers) {
    ...
    (tickers || []).forEach(function (tk) {
      ...
      if (s && s.change_pct != null) {
        var cap = getMarketCap(tk) || 50000;
        sumWeighted += s.change_pct * cap;
        totalCap += cap;
        n += 1;
      }
    });
    return {
      perf: totalCap ? Math.round((sumWeighted / totalCap) * 100) / 100 : 0,
      weight: Math.max(totalCap, 1000),
      count: n
    };
  }
  ...
  function syncSectorFromStocks() {
    ...
    IfluxWatchlistTaxonomy.getGroups('sector').forEach(function (g) {
      ...
      sec.breadth_up = tickers.filter(function (tk) {
        var s = getStock(tk);
        return s && s.change_pct > 0.08;
      }).length;
      sec.breadth_down = tickers.filter(function (tk) {
        var s = getStock(tk);
        return s && s.change_pct < -0.08;
      }).length;
    });
