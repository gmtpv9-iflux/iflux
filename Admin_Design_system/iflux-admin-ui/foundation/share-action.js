/**
 * Foundation — Share Action (Interactive Action)
 * Owner: Design System Foundation (Admin SoT)
 * Không thuộc App Shell. Lazy load khi Widget/Feature cần Share.
 * API: IfluxShareAction (= alias IfluxInsightShare để regression).
 */
(function (global) {
  'use strict';
  ...
  function requireShareLogin() {
    ...
    if (global.ixToast) ixToast('Đăng nhập để chia sẻ link của bạn.', 'warning');
    return false;
  }

  /**
   * P7-API-01 / Brief §6B — entry Share duy nhất (URL artifact).
   * Native Share Sheet khi có; fallback copy Self URL.
   */
  function shareViaNativeSheet(built) {
    if (!built || !built.shareUrl) return Promise.resolve(false);
    if (!global.navigator || typeof navigator.share !== 'function') {
      return Promise.resolve(false);
    }
    var data = {
      url: built.shareUrl,
      title: (built.sharePayload && built.sharePayload.title) || built.title || 'iFlux',
      text: (built.sharePayload && (built.sharePayload.text || built.sharePayload.description)) || ''
    };
    try {
      if (typeof navigator.canShare === 'function' && !navigator.canShare(data)) {
        return Promise.resolve(false);
      }
    } catch (e) { /* ignore canShare quirks */ }
  ...
  function fallbackCopyShareUrl(url) {
    if (!url) return;
    function ok() {
      if (global.ixToast) ixToast('Đã sao chép link của bạn.', 'success');
    }
    function fail() {
      if (global.ixToast) ixToast('Không sao chép được link.', 'warning');
    }
    try {
  ...
  /**
   * Business Goal: Logged-in → Native Sheet → Self URL.
   * @returns {Promise<{ok:boolean, built?:object, reason?:string}>}
   */
  function executeShare(opts) {
    ...
    if (!SF || !SF.buildShareUrl) {
      ...
      return Promise.resolve({ ok: false, reason: 'SHARE_STORE_MISSING' });
    }
    var built;
    try {
      built = SF.buildShareUrl({
        canonicalUrl: opts.canonicalUrl,
        title: opts.title,
        description: opts.description || opts.subtitle,
        image: opts.image,
      ...
      if (global.ixToast) ixToast('Không tạo được link chia sẻ.', 'danger');
      return Promise.resolve({ ok: false, reason: 'SHARE_BUILD_FAILED' });
    }
    return shareViaNativeSheet(built).then(function (usedSheet) {
      if (usedSheet) return { ok: true, built: built, via: 'native-sheet' };
      if (opts.allowFallback === false) return { ok: true, built: built, via: 'none' };
      fallbackCopyShareUrl(built.shareUrl);
      return { ok: true, built: built, via: 'copy-fallback' };
    });
  }

  function stripPublicIdPath(pathname) {
    pathname = String(pathname || '/').split('?')[0].split('#')[0];
    if (!pathname.startsWith('/')) pathname = '/' + pathname;
    var segs = pathname.split('/').filter(Boolean);
    if (segs.length > 0 && PUBLIC_ID_RE.test(segs[0].toUpperCase())) {
      segs.shift();
  ...
  /** Owning page canonical — strip publicId prefix, normalize via Share Foundation. */
  function resolveOwningPageCanonical() {
    var SF = shareFoundation();
    var loc = global.location;
    if (!loc) return '';
    var path = stripPublicIdPath(loc.pathname);
    ...
    return SF && SF.normalizeShareUrl ? SF.normalizeShareUrl(raw) : raw;
  }

  /**
   * Share Boundary R4 — widget owner cung cấp canonical nếu có public route;
   * không có route riêng → owning page canonical.
   */
  function resolveShareCanonical(meta) {
    meta = meta || {};
    var SF = shareFoundation();

    if (meta.entityType === 'widget' && meta.entityId && global.IfluxWidgetRegistry) {
      var reg = IfluxWidgetRegistry.byType(meta.entityId);
      if (reg && reg.shareCanonicalUrl) {
        return SF && SF.normalizeShareUrl
          ? SF.normalizeShareUrl(reg.shareCanonicalUrl)
          : String(reg.shareCanonicalUrl);
      }
    }

    return resolveOwningPageCanonical();
  }
  ...
  function shareBlock(block, def) {
    ...
    var canonicalUrl = resolveShareCanonical(meta);
    if (!canonicalUrl) {
      block.dataset.ifxShareBusy = '0';
      if (global.ixToast) ixToast('Không xác định được link chia sẻ.', 'danger');
      return;
    }

    var SF = shareFoundation();
    if (!SF || !SF.createShare) {
      block.dataset.ifxShareBusy = '0';
  ...
  global.IfluxInsightShare = {
    ...
    openFromPayload: function (payload, bodyEl) {
      ...
      captureElement(bodyEl, profile, function (capture) {
        var canonicalUrl = payload.canonicalUrl || resolveOwningPageCanonical();
        if (!canonicalUrl) {
          if (global.ixToast) ixToast('Không xác định được link chia sẻ.', 'danger');
          return;
        }
        var shareResult;
        try {
          shareResult = IfluxInsightShareStore.createShare({
    ...
    requireShareLogin: requireShareLogin,
    shareViaNativeSheet: shareViaNativeSheet,
    INSIGHT_BLOCK_GROUPS: INSIGHT_BLOCK_GROUPS
  };
  /* Alias Foundation — consumer mới dùng IfluxShareAction */
  global.IfluxShareAction = global.IfluxInsightShare;
})(window);
