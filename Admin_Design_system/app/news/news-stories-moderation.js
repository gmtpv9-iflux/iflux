/* ADM-COM-001 — Kiểm duyệt chủ đề / story posts (news.stories) */
(function (global) {
  'use strict';
  ...
  function load() {
    ...
    var path = '/news/admin/stories/posts?limit=100';
    ...
    request(path).then(function (data) {
      ...
      tbody.innerHTML = list.map(function (p) {
        ...
        var actions = '';
        if (canPerm('news.stories.publish') && p.status !== 'published' && p.status !== 'published_rss') {
        ...
        if (canPerm('news.stories.feature_post')) {
        ...
        if (canPerm('news.stories.pin_post')) {
        ...
        if (canPerm('news.stories.lock_post')) {
        ...
        if (canPerm('news.stories.delete')) {
  ...
  function runAct(act, id) {
    if (act === 'edit') {
      ...
      request('/news/admin/stories/posts/' + encodeURIComponent(id), {
        method: 'PATCH',
    ...
    if (act === 'delete') {
      ...
      request('/news/admin/stories/posts/' + encodeURIComponent(id), { method: 'DELETE' })
        .then(function () { toast('Đã xóa'); load(); })
        .catch(function (e) { toast(e.message, 'danger'); });
      return;
    }
    var path = '/news/admin/stories/posts/' + encodeURIComponent(id) + '/' + act;
  ...
  function boot() {
    ...
    document.getElementById('adm-page-intro') && (document.getElementById('adm-page-intro').textContent =
      'Duyệt / đăng / nổi bật / ghim / khóa bài gắn chủ đề — quyền news.stories.*');
    root.innerHTML =
      '<div class="ix-card">' +
