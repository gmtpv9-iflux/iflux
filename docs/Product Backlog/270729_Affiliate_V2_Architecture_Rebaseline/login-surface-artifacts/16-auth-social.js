  ...
  function startGoogleLoginFromUserGesture(opts) {
    return new Promise(function (resolve, reject) {
      ...
      if (!clickOffscreenGoogleActivator()) {
        ...
        global.__ifxOnGoogleCredential = null;
        clearTimeout(timer);
        reject(new Error('Không mở được cửa sổ Google. Thử trình duyệt khác hoặc cho phép đăng nhập bên thứ ba. [auth-social.js]'));
      }
    });
  }
  ...
  function bindSocialButtons(root, opts) {
    ...
    Object.keys(map).forEach(function (provider) {
      ...
      el.addEventListener('click', function (e) {
        ...
        if (provider === 'google' && googleActivatorReady) {
    ...
    return loadConfig()
      .then(function (c) {
        if (c.google && c.google.enabled) {
          return ensureOffscreenGoogleActivator(c.google).catch(function () { /* optional */ });
        }
      })
      .then(function () {
