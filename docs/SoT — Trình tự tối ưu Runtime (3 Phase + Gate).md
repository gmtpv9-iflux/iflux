...
> **Chuẩn hóa chung (mọi Task/Phase sau này):** [`SoT — Plan Phase Governance (PG-1.0)`](./SoT%20—%20Plan%20Phase%20Governance.md)  
> Bắt buộc: Task Objective → Task Roadmap → Current Phase Objective → Phase Contribution → Exit → mới tới nội dung Phase.  
> Sau khi có Runtime Contract: **PG-008 Contract Rule**.  
> Phase Implementation: **PG-009 Implementation Slice** (Scope · RC · Deliverable · DoD).  
> **Cấm** viết Plan Phase N+1 trước khi Phase N PASS.  
> Mục dưới đây là bản áp dụng cụ thể cho Task Runtime (0→A→B→C→Gate); khi viết Phase mới phải đủ khối PG-001.
