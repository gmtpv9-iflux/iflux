...
**Constraints:** NO CODE · NO Solution · NO SoT · NO ADR · NO implementation proposal

**Trigger:** D5 regression FAIL — Owner test Path C (link `/IFL...` → Đăng nhập header → Google) · không notification · không bảng Affiliate.

**Task pack (canonical index):** [`docs/Product Backlog/Affiliate Attribution Capability/`](Product%20Backlog/Affiliate%20Attribution%20Capability/README.md) — bản trong task pack: [00-Audit-Architecture-Production.md](Product%20Backlog/Affiliate%20Attribution%20Capability/00-Audit-Architecture-Production.md)
...
|---|----------|-------------|
| Q1 | **Login + OAuth tạo account mới** có được coi là signup referral hợp lệ không? | Path C là flow tự nhiên User Web hiện tại |
| Q2 | **Authority model:** `users.referred_by` có phải **唯一** server SoT cho attribution không? | Client parents map tồn tại song song |
| Q3 | **First-touch vs last-touch** khi nhiều capture store (cookie, LS, PNC)? | 3 stores cùng ghi · không có merge policy documented |
| Q4 | **PNC `ownerPublicId`** có liên quan business attribution hay **chỉ** navigation? | Hiện không feed signup |
| Q5 | **Legacy codes** (`MINH10`) — migrate format IFL hay grandfather? | Link 404 / empty builder |
| Q6 | **Admin-created users** có ever bind referral không? | INSERT thiếu column |
| Q7 | **Retroactive bind** — client `linkNewUserToReferrer` sau login có được promote lên server không? | Hiện không · policy undefined |
| Q8 | **Google One Tap** vs **Login Google** vs **Register Google** — ba entry có cùng attribution rule không? | Ba code paths khác nhau |
| Q9 | **Affiliate members table** SoT — server `getAffiliateSync` hay client graph? | UI merge cả hai |
| Q10 | **Notification F0 only** — F1/F2 referral notify có trong scope không? | Consumer code F0 only · task docs mention F1/F2 |

---

## K. Related audit (narrower scope)

**Audit:** Task pack [00-Audit-Architecture-Production.md](../Product%20Backlog/Affiliate%20Attribution%20Capability/00-Audit-Architecture-Production.md) · [00-Audit-D5-R1-Referral-Failure.md](../Product%20Backlog/Affiliate%20Attribution%20Capability/00-Audit-D5-R1-Referral-Failure.md)

---

*End of audit — discovery only. Next step in process: Owner Decision on Open Questions → then Solution → SoT → Plan → Implementation.*
