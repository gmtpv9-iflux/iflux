 ...
 * Projection only: create · replaceProjection · transfer · deactivate
 * Business Transition Authority = IfluxPncLifecycle (not this module)
...
(function (global) {
  ...
  function clone(ctx) {
    ...
    if (ctx.replaced) {
      out.replaced = {
        fromPublicId: ctx.replaced.fromPublicId,
        at: ctx.replaced.at,
        reason: ctx.replaced.reason
      };
    }
  ...
  function getActiveOwner() {
    var ctx = getContext();
    if (!ctx || !ctx.ownerPublicId) return '';
    return String(ctx.ownerPublicId).trim().toUpperCase();
  }

  /**
   * Mirror-only update of guest projection (Lifecycle calls after BD-06 decision).
   * Does not read auth / enforce BD — reject authenticated projection as technical guard.
   */
  function replaceProjection(opts) {
    opts = opts || {};
    var owner = String(opts.ownerPublicId || '').trim().toUpperCase();
    if (!isPublicId(owner) || !activeContext) return null;
    if (activeContext.state === 'authenticated') return null;
    if (activeContext.ownerPublicId === owner) return clone(activeContext);
    var from = activeContext.ownerPublicId;
    activeContext = {
      ownerPublicId: owner,
      source: opts.source || activeContext.source || CONTEXT_SOURCES.INCOMING_PATH,
      state: 'guest',
  ...
  global.IfluxIdentityContext = {
    getActiveOwner: getActiveOwner
  };
