...
> **STRATEGY AMENDMENT — Đóng migration "Production cũ → Staging", mở chiến lược Production mới (2026-08-14):**
> Sau khi hoàn thành `13 - Production ↔ Staging Baseline Verification Report.md` (kết luận **PARTIALLY VERIFIED**), Owner chỉ đạo **đổi chiến lược** so với Phase 7-8 mô tả gốc dưới đây:
> - Task "Production cũ (`iflux.vn`) → Staging (`staging.iflux.vn`)" — **CLOSED**. `staging.iflux.vn` giữ vai trò Reference/Dev, không tiếp tục truy đuổi "giống Production 100%".
> - **Không còn cutover bằng cách biến chính server Production hiện tại thành kiến trúc mới** (khác mô tả gốc ở Phase 8 dòng *"Sau cutover PASS, hạ tầng Production hiện tại chính thức trở thành Production của kiến trúc mới"* — mô tả này **không còn áp dụng**).
...
>
> **FIX BỔ SUNG (2026-08-12, sau báo cáo Owner "vào `staging.iflux.vn` → chuyển trang đăng nhập → 404"):** Root cause thật KHÔNG phải bug CI/CD mới — `iflux-staging.conf` từ trước tới nay chỉ có ~15 location block thủ công (`/api/`, `/media/`, vài `User_Web/...`), **thiếu toàn bộ tầng routing clean-URL tiếng Việt** (`/dang-nhap`, `/dang-ky`, `/co-phieu`, `/nganh`...) mà Production có sẵn trong `snippets/iflux-prod-app.conf` (612 dòng — SSI title, SEO bot-shell, sub_filter runtime script, alias English→VN...). Fix = **sinh bản Staging của đúng file đó** (`snippets/iflux-staging-app.conf`, chỉ đổi `root production→staging` + `proxy_pass 3001→3002`, giữ nguyên logic) rồi `include` vào `iflux-staging.conf` — không viết lại routing logic, không tạo cấu trúc song song. Đồng thời đổi `location = /robots.txt` bản Staging thành `Disallow: /` tĩnh (chặn crawler index môi trường test) + thêm `X-Robots-Tag: noindex, nofollow` cấp server (áp dụng cho location không tự set header riêng).
> Trong lúc verify lại phát hiện tiếp 1 gap Git baseline: `.gitignore` mục "Generated SEO/runtime pages" trước đó exclude **nguyên thư mục** `User_Web/stocks|sectors|ecosystems/` để loại bỏ trang chi tiết per-entity generated (`ACB/`, `1/`, `fpt/`...), nhưng vô tình cuốn theo **`index.html` cấp cha** — SPA shell source thật (byte-giống các trang shell khác, không phải nội dung generate riêng theo ticker) mà 3 route `/co-phieu` `/nganh` `/he-sinh-thai` cần để không 404. Fix = thu hẹp pattern gitignore thành `User_Web/stocks/*/` (chỉ ignore sub-folder theo entity), bổ sung lại 3 file `index.html` từ Production vào Git, commit + push `staging` → CI/CD tự deploy release mới. Đã verify lại toàn bộ 21 route qua domain thật `staging.iflux.vn`: tất cả `200` (trừ `/admin` → `302` bình thường), `iflux.vn` (Production) không đổi.
>
> **OWNER OVERRIDE — BR-23 DB clone (2026-08-14):** `01a - BRD Amendment` cấm clone DB Production→Staging (dòng 45: *"Không tự ý clone/migrate database ngay"*) và Plan 12 lặp lại quyết định này. Owner **chủ động yêu cầu đảo ngược** cho case này, lý do nêu rõ: *"hiện tại Production cũng chỉ mới hoạt động, nếu đưa về Staging thì cũng chỉ là các user test"* — xác nhận qua data thật: `iflux` (Production) chỉ có **9 users**, toàn bộ là tài khoản test, không phải khách hàng thật → rủi ro PII thật không áp dụng cho lần clone này. Agent đã cảnh báo rủi ro (PII, side-effect email/webhook thật, tiền lệ vi phạm văn bản đã ký) bằng `AskQuestion` trước khi thực hiện; Owner xác nhận muốn full raw clone để Staging **giống Production 100%** phục vụ test đầy đủ.
> Thực hiện: backup `iflux_staging` hiện tại có timestamp (`/tmp/iflux_staging_before_clone_<ts>.dump`, rollback point, chưa xoá) → `pg_dump` Production `iflux` (READ-ONLY, không ảnh hưởng Production) → stop PM2 `iflux-api-staging` → `pg_restore --clean --no-owner` **connect bằng chính role `iflux_staging`** (không phải `postgres`/`iflux`) để giữ ownership 100 bảng vẫn thuộc `iflux_staging` sau khi restore — tách biệt credentials theo đúng BR-23 phần *"database credentials"* dù nội dung data đã đồng bộ. Start lại PM2. Verify: row count khớp 1:1 (`users=9`, `community_posts=4015`, `stocks=1526`, `sectors=19`), `iflux_staging` tăng 16MB→700MB (≈ Production 786MB), toàn bộ route qua domain thật `200`, PM2 `iflux-api` (Production) không restart/downtime (uptime 45h liên tục xuyên suốt).
> **Lưu ý còn treo:** các mục Acceptance Criteria khác của BR-23 (database/credentials riêng, config riêng) **vẫn PASS** — chỉ riêng phần "test không đụng dữ liệu Production" được Owner chủ động miễn áp dụng cho **lần clone nội dung này**, không phải huỷ toàn bộ BR-23. Nếu sau này Production có user thật, quyết định này cần được Owner xác nhận lại — không tự động lặp lại clone theo chu kỳ.
