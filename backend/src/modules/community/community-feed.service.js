...
const { ensureSeeded } = require('./community.service');
...
async function fetchCommunityRows(filters) {
  ...
  let sql =
    ...
    ` FROM community_posts WHERE status IN ('published', 'published_rss')`;
...
async function resolveRelatedSeed(relatedTo) {
  ...
  const byId = await query(
    `SELECT id, content_type, status, created_at, ${FEED_PAYLOAD_SQL} AS payload
     FROM community_posts WHERE id = $1 LIMIT 1`,
    [key]
  );
  let row = byId.rows[0];
  if (!row) {
    const bySlug = await query(
      `SELECT id, content_type, status, created_at, ${FEED_PAYLOAD_SQL} AS payload
       FROM community_posts WHERE payload->>'slug' = $1 LIMIT 1`,
      [key]
    );
    row = bySlug.rows[0];
  }
