...
async function ensureSeeded() {
  const res = await query('SELECT COUNT(*)::int AS n FROM community_posts');
  if (res.rows[0].n > 0) return;
  for (const p of SEED_POSTS) {
    await query(
      `INSERT INTO community_posts (id, content_type, status, payload, created_at, updated_at)
...
async function createPost(userId, payload) {
  ...
  await query(
    `INSERT INTO community_posts (id, user_id, content_type, status, payload)
