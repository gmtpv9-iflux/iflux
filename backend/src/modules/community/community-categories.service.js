...
const SELECT_BASE = `
  ...
  LEFT JOIN community_categories p ON p.id = c.parent_id
`;
...
async function deleteCategory(id) {
  ...
  const kids = await query('SELECT COUNT(*)::int AS n FROM community_categories WHERE parent_id = $1', [id]);
  if (kids.rows[0].n > 0) {
    throw AppError.badRequest('CATEGORY_HAS_CHILDREN', 'Hãy xóa hoặc chuyển danh mục con trước');
  }
  await query('DELETE FROM community_categories WHERE id = $1', [id]);
