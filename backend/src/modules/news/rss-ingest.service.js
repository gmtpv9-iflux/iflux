...
async function upsertRssArticle(payload) {
  ...
  if (existing) {
    ...
    const cur = await query('SELECT id, status, payload FROM news_posts WHERE id = $1', [existing.id]);
    ...
    await query(
      `UPDATE news_posts SET status = $2, payload = $3::jsonb, updated_at = NOW() WHERE id = $1`,
  ...
  await query(
    `INSERT INTO news_posts (id, user_id, content_type, status, media_status, payload, created_at, updated_at)
...
async function runRssNewsIngest(opts) {
  opts = opts || {};
...
async function backfillArticleEntities(opts) {
  ...
  for (let i = 0; i < res.rows.length; i++) {
    ...
    try {
      ...
      await query(
        `UPDATE news_posts SET payload = $2::jsonb, updated_at = NOW() WHERE id = $1`,
