...
async function listProviders(filters = {}) {
  const params = [];
  let sql = 'SELECT * FROM news_rss_providers WHERE 1=1';
  if (filters.q) {
...
async function getProvider(id) {
  const res = await query('SELECT * FROM news_rss_providers WHERE id = $1', [id]);
...
async function createProvider(input) {
  ...
  const res = await query(
    `INSERT INTO news_rss_providers (id, name, description, website, rss_index, status)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING *`,
    [
      id,
...
async function deleteProvider(id) {
  const current = await getProvider(id);
  if (!current) throw AppError.notFound('Không tìm thấy nhà cung cấp');
  await query('DELETE FROM news_rss_providers WHERE id = $1', [id]);
